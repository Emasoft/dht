...
class ProfileTestRunner:
    ...
    def run_docker_compose_service(
        self, service: str, compose_file: str = "docker-compose.profiles.yml"
    ) -> tuple[int, str, str]:
    ...
    def print_summary(self) -> None:
        ...
        for profile, result in self.results.items():
            ...
            print(
                f"┃ {profile.upper():<8} ┃ {result['passed']:>6} ┃ {result['failed']:>6} ┃ "
    ...
    def print_profile_differences(self) -> None:
        ...
        if time_diff > 0:
            print(
                f"• Speed: REMOTE was {time_diff:.2f}s faster ({(time_diff / local['duration'] * 100):.1f}% improvement)"
            )
    ...
    def run_development_shell(self, profile: str = "local") -> None:
        ...
        cmd = ["docker", "compose", "-f", "docker-compose.profiles.yml", "run", "--rm", service]
    ...
    def cleanup(self) -> None:
        ...
        cmd = ["docker", "compose", "-f", "docker-compose.profiles.yml", "down", "-v"]
...
def main() -> None:
    ...
    parser.add_argument("--no-cleanup", action="store_true", help="Don't clean up Docker resources after tests")
