...
class TestDuplicateFunctions:
    ...
    def test_no_duplicate_functions_general(self):
        ...
        critical_duplicates = {name: defs for name, defs in duplicates.items() 
                              if name in ['workflows_command']}
        
        assert len(critical_duplicates) == 0, \
            f"Found critical duplicate functions: {critical_duplicates}"
    ...
    def test_workflows_command_properly_located(self):
        ...
        assert 'case "$subcommand" in' in content, \
            "workflows_command should handle subcommands with case statement"
        ...
        for subcommand in essential_subcommands:
            assert f'{subcommand})' in content, \
                f"workflows_command should handle '{subcommand}' subcommand"