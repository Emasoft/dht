...
""")
    ...
    def test_generate_environment_config(self, configurator, sample_project):
        ...
        analysis = {
            ...
            "python_requirements": {"version": "3.11", "dev_packages": ["pip", "setuptools", "wheel"]},
            "quality_setup": {
                "linting": ["ruff"],
                "formatting": ["black"],
                "testing": ["pytest"],
        ...
        # Custom requirements
        custom_reqs = {"quality_tools": ["black", "ruff", "mypy"]}

        config = configurator.generate_environment_config(sample_project, analysis, custom_reqs)
...
class TestEnvironmentInstaller:
    ...
    def test_install_python_environment_with_uv(self, mock_create_venv, mock_check_uv, installer, tmp_path):
        ...
        config = EnvironmentConfig(project_path=tmp_path, project_type="python", python_version="3.11")
        result = ConfigurationResult(success=True, config=config)
