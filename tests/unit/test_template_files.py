...
class TestTemplateFiles:
    ...
    def test_no_template_files_remaining(self):
        ...
        for py_file in python_files:
            try:
                ...
                if has_placeholders:
                    pytest.fail(f"File {py_file.name} still contains template placeholders and should be removed or fixed")
                ...
                for template_file in self.template_files:
                    ...
                    for pattern in import_patterns:
                        assert pattern not in content, \
                            f"File {py_file.name} imports template file {template_file}"
                            
            except UnicodeDecodeError:
                # Skip binary files
                continue
    
    def test_orchestrator_sh_sources_template_files(self):
        """Test whether orchestrator.sh tries to source template files."""
        orchestrator_path = self.dht_modules_dir / "orchestrator.sh"
        if not orchestrator_path.exists():
            pytest.skip("orchestrator.sh not found")
            
        content = orchestrator_path.read_text()
        
        # Check if any template files are sourced
        for template_file in self.template_files:
            assert f"source \"$SCRIPT_DIR/{template_file}\"" not in content, \
                f"orchestrator.sh should not source template file {template_file}"
    ...
    def test_template_files_issue_resolved(self):
        ...
        print(f"  - All Python files in modules/ directory now have valid syntax")
