...
class TestGitHubReposWorkflow:
    ...
    def run_dhtl_command(self, dhtl_path: Path, command: str, args: List[str] = None, 
    ...
    def check_venv_exists(self, repo_dir: Path) -> bool:
        ...
        venv_paths = [
            repo_dir / ".venv",
            repo_dir / "venv",
            repo_dir / ".venv_windows"
        ]
        return any(p.exists() and (p / "bin").exists() or (p / "Scripts").exists() 
                  for p in venv_paths)
    ...
    def run_repository_test(self, dhtl_path: Path, repo_url: str) -> RepoTestResult:
        ...
        success, output, exec_time = self.run_dhtl_command(
            dhtl_path, "clone", [repo_url, repo_name], timeout=120
        )
        ...
        try:
            ...
            if result.setup_success:
                print("📦 Running build...")
                success, output, exec_time = self.run_dhtl_command(
                    dhtl_path, "build", ["--no-checks"], timeout=600
                )
