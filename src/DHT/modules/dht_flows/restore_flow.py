...
def install_dht_dependencies(venv_path: Path) -> dict[str, Any]:
    ...
    dht_deps = ["pyyaml>=6.0", "requests>=2.25.0", "click>=8.0.0", "rich>=10.0.0", "prefect>=2.0.0", "psutil>=5.8.0"]
    ...
    guardian_config = GuardianConfig(memory_limit_mb=1024, timeout_seconds=300, check_interval=1.0)
...
def verify_installation(project_root: Path, venv_path: Path) -> dict[str, Any]:
    ...
    version_result = subprocess.run([str(python_path), "--version"], capture_output=True, text=True)
    ...
    for package in test_imports:
        result = subprocess.run([str(python_path), "-c", f"import {package}"], capture_output=True)
...
def restore_dependencies_flow(
    ...
    install_result = install_dependencies(
        project_root=project_root, venv_path=venv_path, extras=extras, upgrade=upgrade
    )
