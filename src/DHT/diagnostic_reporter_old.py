	...
	•	**Unparsed lines are never lost – they are stored under enumerated keys:

        {
            "python_package_managers": {
                "pipx": {
                    "environment": {
                        "env_vars": { "pipx_home": "/Users/…/.local/pipx" },
...
./diagnostic_reporter.py --output system_bible.json --include-secrets
...
for _pkg in ("psutil", "distro", "requests", "git", "pyyaml"):
    _ensure(_pkg)
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument("--output", default="system_bible.json")
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    Path(args.output).write_text(json.dumps({"system_bible": bible}, indent=2))
    print(f"System bible written to {args.output}")
